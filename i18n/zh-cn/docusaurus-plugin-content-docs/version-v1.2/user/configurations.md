...
| store.db.user                                                    | |
| store.db.password                                                | |
| #store.mode=redis 需要以下配置,该版本redis数据结构被1.3重构,故不推荐在1.2上使用redis存储模式 | |
| store.redis.host                                                 | |
| store.redis.port                                                 | |
...
| transport.compressor            | client和server通信数据压缩方式   |none、gzip、zip、sevenz、bzip2，默认none | |
...
| client.log.exceptionRate                | 日志异常输出概率 |  默认100，目前用于undo回滚失败时异常堆栈输出，百分之一的概率输出，回滚失败基本是脏数据，无限输出堆栈占用硬盘空间  |
...
| client.rm.sqlParserType                | sql解析类型 |  默认druid  |
